...
import string
import random
from evaluation import valuePerPiece, is_forced_move
...
def evalFun(node,space,player):
    return valuePerPiece(node, space, player)

def cutoff(state,depth,space,player):
    all_successors = successors(state, player)
    if is_forced_move(all_successors):
        return 0
    if depth >= 7 or not all_successors:
        return 1
    else:
        return 0
#======================================================
